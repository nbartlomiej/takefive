...
-- isUniform [] = False
-- isUniform (x:[]) = True
-- isUniform (x:xs) = x == (head xs) && (isUniform xs)
...
--   putStr (gameIo input)
-- 
-- gameIo :: String -> String
-- gameIo input = unlines $ map game (lines input)
-- 
-- game :: String -> String
-- game 'q' = "Bye!"
-- game inputs  = "hello world"

-- game :: String -> board -> IO ()
-- game "q" _ = print "Exiting game."
-- game _ board = do
--   print "Hello"
--   updatedInput <-
--   game 
...
{-  mapM_ (\row -> print row) board
  print "(x,y) to put circle, 'q' to exit;"
  c <- getLine
  when (c /= "q") $ do
    main -}
